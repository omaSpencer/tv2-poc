...
const definitions = [
  {
    ...
    steps: [
      ...
      { id: 'catalog-visible', title: 'Katalógus láthatóság', detail: 'Read-only polling a publikus projekcióra.', mode: 'automatic', operation: 'content.catalog-present', timeoutMs: 30_000, expected: [{ kind: 'http', status: 200 }, { kind: 'catalog', present: true }] },
      ...
      { id: 'search-hidden', title: 'Keresésből eltűnt', detail: 'Read-only polling, amíg nincs találat.', mode: 'automatic', operation: 'content.search-absent', timeoutMs: 30_000, expected: [{ kind: 'search', containsContent: false }] },
  ...
  },
  {
    ...
    steps: [
      ...
      { id: 'fallback-a', title: 'Fallback B-re', detail: 'Pontosan egy index marad elérhető és route-olható.', mode: 'automatic', operation: 'outage.processing', expected: [{ kind: 'processing', routeEligible: 1, reachable: 1 }] },
  ...
  },
  {
    ...
    description: 'A dedikált Operations felületen végrehajtandó reindex, karantén/replay és repair ellenőrzőlista; a runner csak állapotot olvas.',
    requiredPermissions: ['ops:read', 'ops:write'], preflight: { authenticated: true, backendReady: true, searchEnabled: true },
    steps: [
      { id: 'reindex', title: 'Reindex A/B', detail: 'Preflight, megerősítés, párhuzamos futás tiltása és sikeres swap.', mode: 'manual', instruction: 'Nyisd meg az Operations / Reindex oldalt, futtasd végig a bizonyított folyamatot, majd erősítsd meg.', expected: [] },
      { id: 'quarantine', title: 'Karantén és replay', detail: 'Hibás elem, javított replay és feldolgozás.', mode: 'manual', instruction: 'Az Operations / Quarantine oldalon futtasd végig a replay folyamatot.', expected: [] },
      { id: 'repair', title: 'Repair mindkét indexen', detail: 'Dry-run, megerősítés és célindexenkénti evidence.', mode: 'manual', instruction: 'Az Operations / Repair oldalon futtasd végig az A és B célindex javítását.', expected: [] },
