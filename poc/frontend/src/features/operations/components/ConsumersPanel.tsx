...
function ConsumerFields({ consumer }: { consumer: Consumer }) {
  return (
    <>
      <div><dt>Pending</dt><dd>{formatInteger(consumer.pending)}</dd></div>
      <div><dt>ACK pending</dt><dd>{formatInteger(consumer.ackPending)}</dd></div>
      <div><dt>ACK floor</dt><dd>{formatInteger(consumer.ackFloorStreamSequence)}</dd></div>
...
export function ConsumersPanel({
  ...
  if (unavailable) {
    return (
      <section className="panel ops-callout ops-callout-warning" role="status">
        <h2>Consumeradatok nem kérhetők le</h2>
  ...
  if (consumers === undefined) {
    return (
      <section className="panel panel-muted" role="status">
        <h2>Consumerek</h2>
        <p>Consumeradat nem érhető el ebben a konfigurációban.</p>
      </section>
    );
  }
  if (consumers.length === 0) {
    return (
      <section className="panel panel-muted" role="status">
        <h2>Consumerek</h2>
        <p>Nincs jelentett consumer. Ez önmagában nem jelent egészséges állapotot.</p>
      </section>
    );
  }
  ...
  return (
    <section className="panel">
      <h2>Durable consumerek</h2>
      <div className="operations-consumer-table table-scroll">
        <table>
          <thead><tr><th>Név</th><th>Pending</th><th>ACK pending</th><th>ACK floor</th><th>Legrégebbi</th><th>Pontos idő</th></tr></thead>
          <tbody>
