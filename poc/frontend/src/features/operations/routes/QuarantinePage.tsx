      ...
      {detail.data ? (
        <section className="panel">
          ...
          <dl className="kv compact-kv">
            ...
            <div><dt>Eredeti sequence</dt><dd>{detail.data.data.originalSequence ?? '—'}</dd></div>
            <div><dt>Durable</dt><dd>{detail.data.data.durable ?? '—'}</dd></div>
          </dl>
          {can(me, 'ops:write') && detail.data.data.schemaValid ? (
            <form className="form-stack" onSubmit={event => { event.preventDefault(); if (reason.trim().length >= 3) replay.mutate(); }}>
              <p className="muted">A replay újra publikálja az eredeti eseményt; a karanténrekord nem törlődik.</p>
              ...
              {replay.error ? <ProblemPanel error={replay.error} title="A replay nem indítható" /> : null}
              <button type="submit" className="btn-danger" disabled={replay.isPending || reason.trim().length < 3}>{replay.isPending ? 'Indítás…' : 'Replay indítása'}</button>
            </form>
          ) : null}
        </section>
      ) : null}
      {actionId ? <OperatorActionPanel actionId={actionId} title="Replay állapota" storageKey={storageKey} /> : null}
    </div>
  );
}
